// src/steps/step6/index.js
// Step 6 – Cỡ mẫu (per-step GPT binding, thống nhất UI)

export const id = 6;
export const title = "Cỡ mẫu";
export const subtitle = "Chọn công thức, nhập giả định; hệ thống tính minh bạch và có bù rớt mẫu.";
...
export async function mount(rootEl, ctx) {
  ...
  rootEl.innerHTML = `
    ...
    <div id="ss">
      ...
      <div class="card-body gpt-row">
        ...
        <button id="ss-gpt-eval"    class="btn btn-secondary" type="button">GPT đánh giá giả định</button>
      </div>
  ...
  async function onSuggest() {
    try {
      toggleBusy(btnSugg, true, 'Đang gợi ý…');
      ...
      const raw = await callAI('step6.suggest', prompt, ctx);
  ...
  async function onEvaluate() {
    try {
      ...
      const raw = await callAI('step6.evaluate', prompt, ctx);
      const md  = String(raw || '').trim();
...
- **Tài liệu gợi ý**: vài nguồn (tên tác giả/năm hoặc guideline) để tra cứu thêm (không cần URL).`);
  ...
  async function callAI(bindingKey, prompt, ctx_) {
    ...
    throw new Error('Chưa cấu hình GPT binding cho step 6');
  }
