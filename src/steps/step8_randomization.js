// src/steps/step8_randomization.js
...
<div class="card">
  ...
  <div class="card-body">
    <div style="font-weight:600;margin-bottom:.5rem">Nhánh can thiệp &amp; Tỷ lệ</div>
    <div id="arm-rows" class="grid-3"></div>
  </div>

  <div class="card-body grid-3" id="block-row" style="display:none">
  ...
  <div class="card-body" id="strata-row" style="display:none">
...
Nữ - ≥65"></textarea>
  ...
  <div class="card-body grid-3">
    ...
    <div class="muted" style="align-self:end">Nếu có N theo nhánh ở bước cỡ mẫu, hệ thống sẽ ưu tiên phân bổ theo tỷ lệ tương ứng.</div>
    <div></div>
  </div>
  ...
  <div class="card-body" id="rand-out" style="display:none">
    <div style="font-weight:600;margin-bottom:.5rem">Preview chuỗi phân bổ</div>
    <div class="muted" id="rand-summary" style="margin-bottom:.5rem"></div>
...
`.trim();
  ...
  // kỳ vọng có thể có: ss.total, ss.perArm:{[armName]: n}
  ...
  // show/hide blocks/strata
  toggleAdvancedRows(methodSel.value);

  // If there is an existing sequence, show preview
...
`.trim();
    ...
    // apply
    if (cfg.method) {
      methodSel.value = ['simple','block','stratified'].includes(cfg.method) ? cfg.method : 'simple';
    }
  ...
  genBtn.addEventListener('click', () => {
    ...
    if (!N || N <= 0) { ctx.toast('Chưa có tổng N hợp lệ.'); return; }
    ...
    if (method === 'simple') {
      ...
      if (!bs.length) { ctx.toast('Vui lòng nhập block sizes hợp lệ.'); return; }
      ...
      const bs = parseBlockSizes(blockSizesI.value); // block-stratified (nếu có)
  ...
  dlBtn.addEventListener('click', () => {
    ...
    if (!seq.length) { ctx.toast('Chưa có chuỗi để tải.'); return; }
    const csv = toCSV(seq);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = Object.assign(document.createElement('a'), { href: url, download: 'randomization_sequence.csv' });
  ...
  function toggleAdvancedRows(method) {
    blockRow.style.display  = method === 'block' || method === 'stratified' ? '' : 'none';
    strataRow.style.display = method === 'stratified' ? '' : 'none';
  }
  ...
  function renderPreview(seq) {
    ...
    seq.forEach((row, i) => {
      ...
      tr.innerHTML = `<td>${i+1}</td><td>${escapeHtml(row.stratum || '')}</td><td>${escapeHtml(row.arm)}</td>`;
  ...
  function readPreview() {
    // không đọc lại từ DOM; trong scope nhỏ, ta tái dựng từ table
    const rows = Array.from(outTblBody.querySelectorAll('tr'));
    return rows.map((tr, i) => {
      const tds = tr.querySelectorAll('td');
      return { index: i+1, stratum: tds[1]?.textContent || '', arm: tds[2]?.textContent || '' };
    });
  }
  ...
  function normalizeRatio(ratioObj, armList) {
    const out = {};
    armList.forEach(a => { out[a] = Math.max(1, parseInt((ratioObj && ratioObj[a]) || '1', 10)); });
    return out;
  }
  ...
  function genSimple(arms, rr, N, seed) {
    const rand = rng(seed);
    const totalRatio = sumRatio(rr);
    // tạo túi theo tỷ lệ, rồi rút ngẫu nhiên đến N
    const bag = [];
  ...
  function genBlock(arms, rr, N, seed, blockSizes) {
    ...
    while (out.length < N) {
      ...
      shuffle(block, rand);
      block.forEach(a => { if (out.length < N) out.push({ index: out.length+1, stratum:'', arm:a }); });
    }
    return out;
  }

  function genStratified(arms, rr, N, seed, strata, blockSizes) {
    // chia N đều theo số strata (nếu không có N theo tầng)
    ...
    strata.forEach((sName, si) => {
      ...
      seq.forEach(r => { r.stratum = sName; r.index = out.length + 1; out.push(r); });
    });
    return out;
  }
